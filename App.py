...
# كود التنسيق وإخفاء معالم الموقع لجعل التطبيق يبدو أصلياً
st.markdown("""
    <style>
...
st.markdown('<div class="hero-section"><h1>🛡️ درع أيمن السيادي</h1><p>الإصدار v20.1 | استقرار تام وأمان فائق</p></div>', unsafe_allow_html=True)
...
with tabs[1]:
    ...
    if scan_choice == "فحص ومعاينة الروابط 🔗":
        u = st.text_input("ألصق الرابط هنا:", key="url_scan_input")
        ...
        u_f = st.file_uploader("ارفع الملف للفحص:", key="main_file_uploader")
        if u_f:
            ...
            if st.button("📁 ابدأ تحليل الملف"):
                ...
                try:
                    ...
                    st.code(content)
                except: st.warning("تعذر عرض محتوى الملف نصياً.")
    st.markdown('</div>', unsafe_allow_html=True)

# 3. التحميل
with tabs[2]:
    v_u = st.text_input("رابط الفيديو للتحميل:", key="video_download_url")
                ...
                try:
                    ...
                    if os.path.exists("ayman_v.mp4"):
                        with open("ayman_v.mp4", "rb") as f:
                            st.video(f.read())
                            st.download_button("📥 حفظ الفيديو على الجهاز", f, "video.mp4")
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("👤 دخول (طلب كود تليجرام)"):
            if pwd == "ayman7716":
                ...
                st.info("تم إرسال الكود إلى تليجرام.")
                st.session_state.failed_attempts = 0
            else:
                st.session_state.failed_attempts += 1
                send_to_telegram(f"⚠️ <b>محاولة دخول خاطئة!</b>\nكلمة السر المستخدمة: <code>{pwd}</code>")
        ...
        if st.session_state.auth_code:
            ...
            if st.button("🔓 تأكيد"):
                if vc == st.session_state.auth_code:
                    ...
                    st.rerun()
                else: st.error("كود التحقق خاطئ")
    else:
        st.success("أهلاً يا أيمن، أنت في لوحة التحكم")
